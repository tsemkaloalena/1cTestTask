...
Procedure ExecuteDataImport() Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when importing data.'; ru = 'Возникли ошибки при загрузке данных.';vi = 'Đã xuất hiện lỗi khi kết nhập dữ liệu.';ro = 'Au apărut erori la importul datelor.'");
			EndIf;
...
Procedure ExecuteDataImportToInformationBase(TableToImport) Export
	...
	MessageString = NStr("en = 'Data exchange process start for node: %1'; ru = 'Начало процесса обмена данными для узла: %1';vi = 'Bắt đầu quá trình trao đổi dữ liệu đối với nút: %1';ro = 'Începutul procesului de schimb de date pentru nod: %1'", CommonUseClientServer.MainLanguageCode());
	...
	// Record in the events log monitor.
	MessageString = NStr("en = '%1, %2; %3 objects are processed'; ru = '%1, %2; Обработано %3 объектов';vi = '%1, %2; Đã xử lý %3 đối tượng';ro = '%1, %2; %3 obiectele sunt procesate'", CommonUseClientServer.MainLanguageCode());
...
Procedure RunExchangeMessageAnalysis(AnalysisParameters = Undefined) Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when analyzing data.'; ru = 'Возникли ошибки при анализе данных.';vi = 'Đã xuất hiện lỗi khi phân tích dữ liệu.';ro = 'Au apărut erori la analizarea datelor.'");
			EndIf;
...
Procedure ExternalConnectionConversionHandlerBeforeDataImport(Cancel) Export
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(22, ErrorDescription(), NStr("en = 'BeforeDataImport (Conversion)'; ru = 'ПередЗагрузкойДанных (конвертация)';vi = 'BeforeDataImport (chuyển đổi)';ro = 'BeforeDataImport (conversie)'"));
			Cancel = True;
		EndTry;
...
Procedure ExternalConnectionAfterDataImport() Export
	...
	If Not ErrorFlag() Then
		...
		If Not IsBlankString(Conversion.AfterDataImport) Then
			...
			Except
				WriteInformationAboutErrorConversionHandlers(23, ErrorDescription(), NStr("en = 'AfterDataImport (conversion)'; ru = 'ПослеЗагрузкиДанных (конвертация)';vi = 'AfterDataImport (chuyển đổi)';ro = 'AfterDataImport (conversie)'"));
			EndTry;
...
Procedure WriteObjectToIB(Object, Type, WriteObject = False, Val SendBack = False) Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ErrorMessageString = NStr("en = 'Attempting to change shared data (%1) in split session.'; ru = 'Попытка изменения неразделенных данных (%1) в разделенном сеансе.';vi = 'Thử thay đổi dữ liệu chưa chia tách (%1) trong phiên làm việc đã chia tách.';ro = 'Încercarea de a schimba datele partajate (%1) în sesiunea separată.'");
...
Function DumpByRule(Source					= Undefined,
	...
	If HasBeforeConvertObjectGlobalHandler Then
		...
		Except
			WriteInformationAboutOCRHandlerErrorDump(64, ErrorDescription(), OCR, Source, NStr("en = 'BeforeObjectConversion (global)'; ru = 'ПередКонвертациейОбъекта (глобальный)';vi = 'BeforeObjectConversion (toàn cục)';ro = 'BeforeObjectConversion (la nivel mondial)'"));
		EndTry;
	...
	If Not DontExportByValueMap Then
		...
		|vi = 'Trong các quy tắc chuyển đổi giá trị (QT3) cần so sánh giá trị Nguồn với giá trị Đích.
		|Nếu không có giá trị đích thích hợp, thì chỉ ra giá trị trống.';
		|ro = 'În regula de conversie a valorilor (VCR) trebuie să se potrivească valoarea Sursă cu valoarea Receiver.
		|Dacă nu există nicio valoare adecvată pentru receptor, specificați o valoare goală.'");
				//
...
Procedure DumpRegister(RecordSetForExport, 
	...
	For Each FilterRow IN RecordSetFilter Do
		...
		If PCRRow <> Undefined
			...
			KeyProperty = ?(IsBlankString(PCRRow.Source), PCRRow.Receiver, PCRRow.Source);
			
			SourceStructure.Insert(KeyProperty, FilterRow.Value);
		...
		ErrorPresentation = StringFunctionsClientServer.SubstituteParametersInString(
								NStr("en = 'An error occurred while opening file for exchange message writing.
								     ...
								     |ru = 'Ошибка при открытии файла для записи сообщения обмена.
										|Имя файла %1.
										|Описание ошибки: %2';
										|vi = 'Lỗi khi mở tệp để ghi lại thông báo trao đổi.
										|Tên tệp %1.
										|Mô tả lỗi: %2';
										|ro = 'A apărut o eroare la deschiderea fișierului pentru scrierea mesajului de schimb.
										|Numele fișierului %1.
										| Descrierea erorii: %2'"),
...
Procedure ExchangeProtocolInitialization()
	...
	Except
		DataLogFile = Undefined;
		MessageString = NStr("en = 'An error occurred when attempting to write to data protocol file: %1. Error description: %2'; ru = 'Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';vi = 'Lỗi khi thử ghi ra tệp giao thức dữ liệu: %1. Mô tả lỗi: %2';ro = 'A apărut o eroare la încercarea de scriere în fișierul protocolului de date: %1. Descrierea erorii: %2'",
...
Procedure RestoreRulesFromInternalFormat() Export
	...
	If StorageRulesFormatVersion <> FormatVersionStorageRulesExchange() Then
		...
		|vi = 'Phiên bản định dạng lưu quy tắc trao đổi không như phiên bản dự tính.
		|Cần thực hiện kết nhập lại quy tắc trao đổi.';
		|ro = 'Versiunea formularului de stocare a regulilor de schimb nu corespunde celei așteptate.
...
Procedure StartMessageReader(MessageReader, DataAnalysis = False)
	...
	If ExchangeFile.NodeType <> XMLNodeType.StartElement Then
		Raise NStr("en = 'Error in exchange message format.'; ru = 'Ошибка формата сообщения обмена.';vi = 'Lỗi định dạng thông báo trao đổi.';ro = 'Eroare în formatul mesajului de schimb.'");
	EndIf;
	...
	If ConversionRulesTable.Count() = 0 Then
		...
		If ErrorFlag() Then
			Raise NStr("en = 'Errors occurred when importing data exchange rules.'; ru = 'При загрузке правил обмена данными возникли ошибки.';vi = 'Khi kết nhập quy tắc trao đổi dữ liệu đã nảy sinh lỗi.';ro = 'Au apărut erori la importul regulilor de schimb de date.'");
		EndIf;
	Else
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(22, ErrorDescription(), NStr("en = 'BeforeDataImport (Conversion)'; ru = 'ПередЗагрузкойДанных (конвертация)';vi = 'BeforeDataImport (chuyển đổi)';ro = 'BeforeDataImport (conversie)'"));
		EndTry;
		
		If Cancel Then
			Raise NStr("en = 'Cancel importing exchange message in the BeforeDataImport handler (conversion).'; ru = 'Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';vi = 'Từ chối kết nhập thông điệp trao đổi trong hàm sự kiện BeforeDataImport (chuyển đổi).';ro = 'Anulați importul mesajului de schimb în handler BeforeDataImport (conversie).'");
		EndIf;
	...
	If ExchangeFile.LocalName = "DataTypeInfo" Then
		...
		If ExchangeFile.NodeType <> XMLNodeType.StartElement Then
			Raise NStr("en = ""Error in exchange message's format.""; ru = 'Ошибка формата сообщения обмена.';vi = 'Lỗi định dạng thông báo trao đổi.';ro = 'Eroare în formatul mesajului de schimb.'");
		EndIf;
		...
		Else
			...
			If ErrorFlag() Then
				Raise NStr("en = 'Errors occurred when importing information on data types.'; ru = 'При загрузке информации о типах данных возникли ошибки.';vi = 'Khi kết nhập thông tin về kiểu dữ liệu, đã nảy sinh lỗi.';ro = 'Au apărut erori la importul de informații privind tipurile de date.'");
			EndIf;
		EndIf;
		ExchangeFile.Read();
	EndIf;
	...
	If TransactionActive() Then
		Raise NStr("en = 'Lock of data receipt cannot be set in an active transaction.'; ru = 'Блокировка получения данных не может быть установлена в активной транзакции.';vi = 'Không thể khóa việc nhận dữ liệu ở giao tác hiện hoạt.';ro = 'Blocarea primirii datelor nu poate fi setată într-o tranzacție activă.'");
	EndIf;
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en = 'Setting lock on data exchange error.
				 ...
				 |ru = 'Ошибка установки блокировки на обмен данными.
					...
					|
					|Подробности: %1';
					|vi = 'Lỗi cài đặt phong tỏa trao đổi dữ liệu.
					|Có thể, việc trao đổi dữ liệu được thực hiện bởi phiên làm việc khác.
					|
					|Chi tiết:%1';
					|ro = 'Setarea blocării erorilor de schimb de date.
					|Schimbul de date poate fi efectuat printr-o altă sesiune.
					|
					|Detalii: %1'"),
...
Procedure FinishMessageReading(Val MessageReader)
	...
	If Not IsBlankString(Conversion.AfterDataImport) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(23, ErrorDescription(), NStr("en = 'AfterDataImport (conversion)'; ru = 'ПослеЗагрузкиДанных (конвертация)';vi = 'AfterDataImport (chuyển đổi)';ro = 'AfterDataImport (conversie)'"));
		EndTry;
	...
	Try
		...
		If Cancel = True Then
			...
			If Not IsBlankString(CancelReason) Then
				...
				MessageString = NStr("en = 'Decline exchange message import in handler AfterParametersImport (conversion) due to: %1'; ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';vi = 'Từ chối kết nhập thông báo trao đổi trong hàm sự kiện AfterParametersImport (chuyển đổi) theo nguyên nhân: %1';ro = 'Refuză importul mesajului de schimb în handler AfterParametersImport (conversie) din cauza: %1'");
				MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
				Raise MessageString;
			Else
				Raise NStr("en = 'Cancel importing exchange message in the AfterImportOfParameters handler (conversion).'; ru = 'Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';vi = 'Từ chối kết nhập thông báo trao đổi trong hàm sự kiện AfterImportOfParameters (chuyển đổi).';ro = 'Anulați importul mesajului de schimb în instrumentul handler AfterImportOfParameters (conversie).'");
			EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject)
	...
	For Each RegisterRecordRow IN RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			ErrorMessage = NStr("en = 'Access right violation: %1'; ru = 'Нарушение прав доступа: %1';vi = 'Vi phạm quyền truy cập: %1';ro = 'Încălcarea dreptului de acces: %1'");
	...
	For Each RegisterRecordRow IN RecordTableRowToProcessArray Do		
		...
		Except
			...
			ErrorMessage = NStr("en = 'Operation is not executed: %1 %2'; ru = 'Операция не выполнена: %1 %2';vi = 'Đã thực hiện giao dịch: %1 %2';ro = 'Operația nu este executată: %1 %2'");
	...
	While Variantsearchnumber <= 10
		...
		Except
			...
			WriteInformationAboutOCRHandlerErrorImport(73, ErrorDescription(), "", "", 
				ObjectType, Undefined, NStr("en = 'Sequence of search fields'; ru = 'Последовательность полей поиска';vi = 'Chuỗi nối tiếp các trường tìm kiếm';ro = 'Secvența câmpurilor de căutare'"));
...
Procedure ImportTabularSection(Object, TabularSectionName, DocumentTypeCommonInformation, ObjectParameters, OCR)
	...
	Except
		...
		Text = NStr("en = 'Tabular section name: %1'; ru = 'Имя табличной части: %1';vi = 'Tên phần bảng: %1';ro = 'Nume secțiune de tabel:  %1'");
		...
		ElsIf NodeName = "ExtDimensionDr" OR NodeName = "ExtDimensionCr" Then
			...
			While ExchangeFile.Read() Do
				...
				If NodeName = "Property" Then
					...
					If Name = "Key" Then
						...
						KeyProperty = ReadProperty(PropertyType);
			...
			If KeyProperty <> Undefined 
				...
				If Not MustSearchInTS Then
					...
					Record[NodeName][KeyProperty] = Value;
				...
				Else
					...
					RecordMapping.Insert(KeyProperty, Value);
...
Function ImportObjectTypes(Source)
	...
	AllowedLengthProperty =  deAttribute(Source, StringType, "AllowedLength");
	...
	If TypeArray.Count() > 0 Then
		...
		If Length > 0 Then
			If AllowedLengthProperty = "Fixed" Then
				AllowedLengthProperty = AllowedLength.Fixed;
			Else
				AllowedLengthProperty = AllowedLength.Variable;
			EndIf;
...
Procedure CommentsToObjectImport(NPP, Rulename, Source, ObjectType, GNPP = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("en = 'Import object No. %1'; ru = 'Загрузка объекта № %1';vi = 'Kết nhập đối tượng số %1';ro = 'Import obiecte Nr. %1'");
...
Function ReadRegisterRecordSet()
	...
	If HasBeforeObjectImportGlobalHandler Then
		...
		Except
			...
			ObjectType, Undefined, NStr("en = 'BeforeObjectImport (global)'; ru = 'ПередЗагрузкойОбъекта (глобальный)';vi = 'BeforeObjectImport (toàn cục)';ro = 'BeforeObjectImport (global)'"));
			...
			If HasAftertObjectImportGlobalHandler Then
				...
				Except
					...
					WriteInformationAboutOCRHandlerErrorImport(54, ErrorDescription(), Rulename, Source,
						ObjectType, Object, NStr("en = 'AftertObjectImport (global)'; ru = 'ПослеЗагрузкиОбъекта (глобальный)';vi = 'AftertObjectImport (toàn cục)';ro = 'AftertObjectImport (global)'"));
			...
			If ObjectTypeName = "Document" Then
				...
				If DataImportToInformationBaseMode() Then
					...
					Except
						...
						MessageString = NStr("en = 'An error occurred while writing document: %1 Error description: %2'; ru = 'Ошибка при записи документа: %1. Описание ошибки: %2';vi = 'Lỗi khi ghi chứng từ: %1. Mô tả lỗi: %2';ro = 'A apărut o eroare la scrierea documentului: %1 Descrierea erorii: %2'");
...
Procedure DumpGroupOfProperties(Source, Receiver, IncomingData, OutgoingData, OCR, PGCR, PropertyCollectionNode, 
	...
	If ExportGroupToFile Then
		...
		|Описание
		|ошибки: %2';
		|vi = 'Lỗi khi tạo tệp tạm thời để kết nhập dữ liệu
		|Tên tệp""%1"".
		|Mô tả
		|lỗi: %2';
		|ro = 'A apărut o eroare la crearea fișierului temporar pentru importul de date.
		|Numărul fișierului ""%1""
		|.Descrierea
		|erorii: %2'"),
...
Procedure CallEventsBeforeObjectExport(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If CommentObjectProcessingFlag Then
		...
		EventName = NStr("en = 'ObjectExport: %1'; ru = 'ВыгрузкаОбъекта: %1';vi = 'ObjectExport: %1';ro = 'ExportObiect: %1'");
	...
	If HasBeforeObjectExportGlobalHandler Then
		...
		Except
			WriteInformationAboutErrorDDRHandlers(65, ErrorDescription(), Rule.Name, NStr("en = 'BeforeExportSelectionObject (global)'; ru = 'ПередВыгрузкойОбъектаВыборки (глобальный)';vi = 'BeforeExportSelectionObject (toàn cục)';ro = 'BeforeExportSelectionObject (global)'"), Object);
		EndTry;
...
Procedure CallEventsAfterObjectExport(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If HasAfterObjectExportGlobalHandler Then
		...
		Except
			WriteInformationAboutErrorDDRHandlers(69, ErrorDescription(), Rule.Name, NStr("en = 'AfterSelectionObjectExport (Global)'; ru = 'ПослеВыгрузкиОбъектаВыборки (глобальный)';vi = 'AfterSelectionObjectExport (toàn cục)';ro = 'AfterSelectionObjectExport (Global)'"), Object);
		EndTry;
	EndIf;
...
Procedure DumpDataByRule(Rule) Export
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("en = 'DATA EXPORT RULE: %1 (%2)'; ru = 'ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';vi = 'QUY TẮC KẾT XUẤT DỮ LIỆU: %1 (%2)';ro = 'REGULA DE EXPORT A DATELOR: %1 (%2)'");
...
Procedure WorkThroughObjectDeletion(ObjectDeletionData, ErrorMessageString = "")
	...
	If Conversion.Property("BeforeSendDeletionInfo", EventText) Then
		...
		If Not IsBlankString(EventText) Then
			...
			Except
				ErrorMessageString = WriteInformationAboutErrorConversionHandlers(76, ErrorDescription(), NStr("en = 'BeforeSendingInformationAboutDeletion (conversion)'; ru = 'ПередОтправкойИнформацииОбУдалении (конвертация)';vi = 'BeforeSendingInformationAboutDeletion (chuyển đổi)';ro = 'ÎnainteaTrimiteriiInformațiilorDespreȘtergere (conversie)'"));
...
Function GetRegistrationAttributesStructure(PCRTable)
	...
	For Each PCR IN PCRRowArray Do
		...
		Except
			WriteLogEvent(NStr("en = 'Data exchange.Conversion rule import'; ru = 'Обмен данными.Загрузка правил конвертации';vi = 'Trao đổi dữ liệu.Kết nhập quy tắc chuyển đổi';ro = 'Schimb de date.Import de reguli de conversie'", CommonUseClientServer.MainLanguageCode()),
...
Procedure RunObjectRegistrationAttributesCheck(ChangeRecordAttributeTable)
	...
	For Each TableRow IN ChangeRecordAttributeTable Do
		...
		Except
			...
			MessageString = NStr("en = 'Object type is undefined: %1'; ru = 'Тип объекта не определен: %1';vi = 'Chưa xác định kiểu đối tượng: %1';ro = 'Tipul de obiect este nedefinit: %1'");
			...
			For Each Attribute IN TableRow.ChangeRecordAttributeStructure Do
				...
				If CommonUse.ThisIsTask(MDObject) Then
					...
					If Not (MDObject.Attributes.Find(Attribute.Key) <> Undefined
						...
						MessageString = NStr("en = 'Attributes of ""%1"" object header are specified incorrectly. Attribute ""%2"" does not exist.'; ru = 'Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.';vi = 'Chỉ ra chưa chính xác mục tin phần đầu của đối tượng ""%1"". Mục tin ""%2"" không tồn tại.';ro = 'Atributele antetului obiect ""%1"" sunt specificate incorect. Atributul ""%2"" nu există.'");
		...
		Else
			...
			If MetaTable = Undefined Then
				WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en = 'Tabular section (standard tabular section, movements) %1 of object %2 does not exist.'; ru = 'Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';vi = 'Phần bảng (phần bảng chuẩn, luân chuyển) ""%1"" của đối tượng ""%2"" không tồn tại.';ro = 'Secțiunea tabelară (secțiunea tabelară standard, mișcări) %1 a obiectului %2 nu există.'"),
			...
			For Each Attribute IN TableRow.ChangeRecordAttributeStructure Do
				...
				If Not AttributeFoundInObjectsRegistrationAttributesTabularSection(MetaTable, Attribute.Key) Then
					WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en = 'Attribute ""%3"" does not exist in the table section (standard table section, motion) ""%1"" of object ""%2"".'; ru = 'Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';vi = 'Mục tin ""%3"" không tồn tại trong phần bảng (phần bảng chuẩn, luân chuyển) ""%1"" của đối tượng ""%2"".';ro = 'Atributul ""%3"" nu există în secțiunea tabelă (secțiunea tabelară standard, mișcare) ""%1"" a obiectului ""%2"".'"),
...
Function GetInformationAboutRules(AreCorrespondentRules = False) Export
	...
	If AreCorrespondentRules Then
		InfoString = NStr("en = 'Conversion rules of correspondent (%1) from %2'; ru = 'Правила конвертации корреспондента (%1) от %2';vi = 'Quy tắc chuyển đổi nơi (%1) từ %2';ro = 'Normele de conversie ale corespondentului (%1) de la %2'");
	Else
		InfoString = NStr("en = 'Conversion rules of this infobase (%1) from %2'; ru = 'Правила конвертации этой информационной базы (%1) от %2';vi = 'Quy tắc chuyển đổi của cơ sở thông tin này (%1) từ %2';ro = 'Regulile de conversie ale acestei baze de date (%1) de la %2'");
	EndIf;
...
Procedure ImportExchangeRules(Source="",
	...
	If ExchangeMode <> "Import" AND Conversion.Property("AfterExchangeRuleImport", AfterExchangeRuleImportEventText)
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(75, ErrorDescription(), NStr("en = 'AfterImportOfExchangeRules (conversion)'; ru = 'ПослеЗагрузкиПравилОбмена (конвертация)';vi = 'AfterImportOfExchangeRules (chuyển đổi)';ro = 'AfterImportOfExchangeRules (conversie)'"));
			Cancel = True;
...
Procedure DeleteObjectByRef(Ref, ErrorMessageString)
	...
	If Not IsBlankString(Conversion.OnGetDeletionInfo) Then
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(77, ErrorDescription(), NStr("en = 'WhenGettingInformationAboutDeleting (Conversion)'; ru = 'ПриПолученииИнформацииОбУдалении (конвертация)';vi = 'WhenGettingInformationAboutDeleting (chuyển đổi)';ro = 'WhenGettingInformationAboutDeleting (Conversie)'"));
			Cancel = True;
					...
					If TableRow = Undefined Then
						...
						For Each Column IN ExchangeMessageDataTable.Columns Do
							...
							If Column.ValueType.ContainsType(StringType) Then
								...
								TableRow[Column.Name] = NStr("en = 'Deleting object'; ru = 'Удаление объекта';vi = 'Xóa đối tượng';ro = 'Ștergerea obiectului'");
				...
				Try
					...
					If Cancel = True Then
						...
						If Not IsBlankString(CancelReason) Then
							...
							MessageString = NStr("en = 'Data import was canceled as: %1'; ru = 'Загрузка данных отменена по причине: %1';vi = 'Đã hủy bỏ kết nhập dữ liệu theo nguyên nhân: %1';ro = 'Importul de date a fost anulat ca: %1'");
							MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
							Raise MessageString;
						Else
							Raise NStr("en = 'Data import is canceled'; ru = 'Загрузка данных отменена';vi = 'Đã hủy bỏ kết nhập dữ liệu';ro = 'Import de date este anulat'");
						EndIf;
...
Procedure ReadExchangeData(MessageReader, DataAnalysis)
	...
	If Not ValueIsFilled(ExchangeNodeRecipient)
		...
		MessageString = NStr("en = 'Exchange node for data import is not found. Exchange plan: %1, Code: %2'; ru = 'Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2';vi = 'Chưa tìm thấy nút trao đổi để kết nhập dữ liệu. Sơ đồ trao đổi: %1, Mã: %2';ro = 'Nodul de schimb pentru importul de date nu a fost găsit. Planul de schimb: %1, Cod: %2'");
	...
	If DataImportToInformationBaseMode() Then
		...
		If ReceivedNo >= MessageReader.MessageNo Then // Message number is either smaller or equals to the previously accepted one.
			...
			Raise NStr("en = 'Exchange message was previously received'; ru = 'Сообщение обмена было принято ранее';vi = 'Đã tiếp nhận thông điệp trao đổi trước đây';ro = 'Mesajul de schimb a fost primit anterior'");
		EndIf;
		...
		If DeleteChangeRecords Then // Delete registration of changes.
			...
			If TransactionActive() Then
				Raise NStr("en = 'Deletion of the data change registration cannot be completed in an active transaction.'; ru = 'Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';vi = 'Không thể thực hiện xóa nhật ký thay đổi dữ liệu ở giao tác hiện thời.';ro = 'Ștergerea înregistrării modificărilor de date nu poate fi finalizată într-o tranzacție activă.'");
			EndIf;
	...
	If Not IsBlankString(Conversion.AfterGetExchangeNodeDetails) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(176, ErrorDescription(), NStr("en = 'AfterReceivingInformationAboutExchangeNodes (conversion)'; ru = 'ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';vi = 'AfterReceivingInformationAboutExchangeNodes (chuyển đổi)';ro = 'AfterReceivingInformationAboutExchangeNodes (conversie)'"));
		EndTry;
	...
	For Each MappingObject IN ObjectsForPostponedRecording() Do
		...
		Try
			...
			Else
				...
				ErrorDescription = NStr("en = 'An error occurred when checking the attribute population'; ru = 'Ошибка проверки заполнения реквизитов';vi = 'Lỗi kiểm tra việc điền mục tin';ro = 'A apărut o eroare la verificarea completării atributelor'");
...
Procedure RunExport(ErrorMessageString = "")
	...
	If Not IsBlankString(Conversion.BeforeDataExport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(62, ErrorDescription(), NStr("en = 'BeforeDataExport (conversion)'; ru = 'ПередВыгрузкойДанных (конвертация)';vi = 'BeforeDataExport (chuyển đổi)';ro = 'BeforeDataExport (conversie)'"));
			Cancel = True;
		EndTry; 
	...
	If Not Cancel AND Not IsBlankString(Conversion.AfterDataExport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(63, ErrorDescription(), NStr("en = 'AfterDataExport (conversion)'; ru = 'ПослеВыгрузкиДанных (конвертация)';vi = 'AfterDataExport (chuyển đổi)';ro = 'După depozit de date (conversie)'"));
		EndTry;
...
Procedure RunDumpOfRegisteredData(WriteMessage, ErrorMessageString, UsedUnloadRulesTable)
	...
	If Not IsBlankString(Conversion.BeforeGetChangedObjects) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(175, ErrorDescription(), NStr("en = 'BeforeReceivingChangedObjects (conversion)'; ru = 'ПередПолучениемИзмененныхОбъектов (конвертация)';vi = 'BeforeReceivingChangedObjects (chuyển đổi)';ro = 'BeforeReceivingChangedObjects (conversie)'"));
			Return;
		EndTry;
...
Procedure MessagesInitialization()
	...
	ErrorMessages.Insert(2,  NStr("en = 'An error occurred when unpacking an exchange file. The file is locked.'; ru = 'Ошибка распаковки файла обмена. Файл заблокирован.';vi = 'Lỗi khi giải nén tệp trao đổi. Tệp bị khóa.';ro = 'A apărut o eroare la dezarhivarea unui fișier de schimb. Fișierul este blocat.'"));
	ErrorMessages.Insert(3,  NStr("en = 'The specified exchange rule file does not exist.'; ru = 'Указанный файл правил обмена не существует.';vi = 'Tệp quy tắc trao đổi đã chọn không tồn tại.';ro = 'Fișierul de reguli de schimb specificat nu există.'"));
	ErrorMessages.Insert(4,  NStr("en = 'An error occurred when creating COM object Msxml2.DOMDocument'; ru = 'Ошибка при создании COM-объекта Msxml2.DOMDocument';vi = 'Lỗi khi tạo đối tượng COM Msxml2.DOMDocument';ro = 'A apărut o eroare la crearea COM object Msxml2.DOMDocument'"));
	ErrorMessages.Insert(5,  NStr("en = 'An error occurred when opening the exchange file'; ru = 'Ошибка открытия файла обмена';vi = 'Lỗi mở tệp trao đổi';ro = 'A apărut o eroare la deschiderea fișierului de schimb'"));
	ErrorMessages.Insert(6,  NStr("en = 'An error occurred when importing exchange rules'; ru = 'Ошибка при загрузке правил обмена';vi = 'Lỗi khi kết nhập quy tắc trao đổi';ro = 'A apărut o eroare la importarea regulilor de schimb'"));
	ErrorMessages.Insert(7,  NStr("en = 'Error in exchange rule format'; ru = 'Ошибка формата правил обмена';vi = 'Lỗi định dạng quy tắc trao đổi';ro = 'Eroare în formatul regulii de schimb'"));
	ErrorMessages.Insert(8,  NStr("en = 'Incorrect file name for data export'; ru = 'Не корректно указано имя файла для выгрузки данных';vi = 'Đã chỉ ra không đúng tên tệp để kết xuất dữ liệu';ro = 'Numele de fișier incorect pentru exportul de date'")); // not used
	ErrorMessages.Insert(9,  NStr("en = 'Error in exchange file format'; ru = 'Ошибка формата файла обмена';vi = 'Lỗi định dạng tệp trao đổi';ro = 'Eroare în formatul fișierului de schimb'"));
	ErrorMessages.Insert(10, NStr("en = 'File name for data export is not specified (Data file name)'; ru = 'Не указано имя файла для выгрузки данных (Имя файла данных)';vi = 'Chưa chỉ ra tên tệp để kết xuất dữ liệu (Tên tệp dữ liệu)';ro = 'Numele fișierului pentru exportul de date nu este specificat (numele fișierului de date)'"));
	ErrorMessages.Insert(11, NStr("en = 'Link to a non-existing metadata object in the exchange rules'; ru = 'Ссылка на несуществующий объект метаданных в правилах обмена';vi = 'Tham chiếu đến đối tượng Metadata không tồn tại trong các quy tắc trao đổi';ro = 'Link la un obiect de metadate inexistent în regulile de schimb'"));
	ErrorMessages.Insert(12, NStr("en = 'File name with exchange rules is not specified (Rule file name)'; ru = 'Не указано имя файла с правилами обмена (Имя файла правил)';vi = 'Chưa chỉ ra tên tệp với các quy tắc trao đổi (Tên tệp quy tắc)';ro = 'Numele fișierului cu reguli de schimb nu este specificat (numele fișierului de reguli)'"));
			
	ErrorMessages.Insert(13, NStr("en = 'An error occurred when receiving a value of the object property (by the name of the source property)'; ru = 'Ошибка получения значения свойства объекта (по имени свойства источника)';vi = 'Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính nguồn)';ro = 'A apărut o eroare la primirea unei valori a proprietății obiectului (după numele proprietății sursei)'"));
	ErrorMessages.Insert(14, NStr("en = 'An error occurred when receiving the object property value (by the target property name)'; ru = 'Ошибка получения значения свойства объекта (по имени свойства приемника)';vi = 'Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính đích)';ro = 'A apărut o eroare la primirea valorii proprietății obiectului (după numele proprietății țintă)'"));
	
	ErrorMessages.Insert(15, NStr("en = 'File name for data import is not specified (File name for import)'; ru = 'Не указано имя файла для загрузки данных (Имя файла для загрузки)';vi = 'Chưa chỉ ra tên tệp để kết nhập dữ liệu (Tên tệp để kết nhập)';ro = 'Numele fișierului pentru importul de date nu este specificat (numele fișierului pentru import)'"));
			
	ErrorMessages.Insert(16, NStr("en = 'An error occurred while receiving value of subobject property (by source property name)'; ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';vi = 'Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính nguồn)';ro = 'A apărut o eroare la primirea valorii proprietatii subobiectului (după numele proprietății sursă)'"));
	ErrorMessages.Insert(17, NStr("en = 'An error occurred while receiving value of subobject property (by target property name)'; ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';vi = 'Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính đích)';ro = 'A apărut o eroare în timpul primirii valorii proprietății subiectului (după numele proprietății țintă)'"));
	ErrorMessages.Insert(18, NStr("en = 'An error occurred when creating a data processor with the handler code'; ru = 'Ошибка при создании обработки с кодом обработчиков';vi = 'Lỗi khi tạo mới bộ xử lý có mã hàm sự kiện';ro = 'A apărut o eroare la crearea unui procesor de date cu codul de handler'"));
	ErrorMessages.Insert(19, NStr("en = 'An error occurred in event handler BeforeObjectImport'; ru = 'Ошибка в обработчике события ПередЗагрузкойОбъекта';vi = 'Lỗi trong hàm sự kiện BeforeObjectImport';ro = 'A apărut o eroare în handler-ul evenimentului BeforeObjectImport'"));
	ErrorMessages.Insert(20, NStr("en = 'An error occurred in event handler OnObjectImport'; ru = 'Ошибка в обработчике события ПриЗагрузкеОбъекта';vi = 'Lỗi trong hàm sự kiện OnObjectImport';ro = 'A apărut o eroare în handler-ul evenimentului OnObjectImport'"));
	ErrorMessages.Insert(21, NStr("en = 'An error occurred in event handler AfterObjectImport'; ru = 'Ошибка в обработчике события ПослеЗагрузкиОбъекта';vi = 'Lỗi trong hàm sự kiện AfterObjectImport';ro = 'A apărut o eroare în handler-ul evenimentului AfterObjectImport'"));
	ErrorMessages.Insert(22, NStr("en = 'An error occurred in event handler BeforeDataImport (conversion)'; ru = 'Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';vi = 'Lỗi trong hàm sự kiện BeforeDataImport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului BeforeDataImport (conversie)'"));
	ErrorMessages.Insert(23, NStr("en = 'An error occurred in event handler AfterDataImport (conversion)'; ru = 'Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';vi = 'Lỗi trong hàm sự kiện AfterDataImport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului AfterDataImport (conversie)'"));
	ErrorMessages.Insert(24, NStr("en = 'An error occurred when removing an object'; ru = 'Ошибка при удалении объекта';vi = 'Lỗi khi xóa đối tượng';ro = 'A apărut o eroare la eliminarea unui obiect'"));
	ErrorMessages.Insert(25, NStr("en = 'An error occurred when writing the document'; ru = 'Ошибка при записи документа';vi = 'Lỗi khi ghi chứng từ';ro = 'A apărut o eroare la scrierea documentului'"));
	ErrorMessages.Insert(26, NStr("en = 'An error occurred when writing the object'; ru = 'Ошибка записи объекта';vi = 'Lỗi ghi đối tượng';ro = 'A apărut o eroare la scrierea obiectului'"));
	ErrorMessages.Insert(27, NStr("en = 'An error occurred in event handler BeforeProcessClearingRule'; ru = 'Ошибка в обработчике события ПередОбработкойПравилаОчистки';vi = 'Lỗi trong hàm sự kiện BeforeProcessClearingRule';ro = 'A apărut o eroare în handler-ul evenimentului BeforeProcessClearingRule'"));
	ErrorMessages.Insert(28, NStr("en = 'An error occurred in event handler AfterClearingRuleProcessing'; ru = 'Ошибка в обработчике события ПослеОбработкиПравилаОчистки';vi = 'Lỗi trong hàm sự kiện AfterClearingRuleProcessing';ro = 'A apărut o eroare în handler-ul evenimentului AfterClearingRuleProcessing '"));
	ErrorMessages.Insert(29, NStr("en = 'An error occurred in event handler BeforeDeleteObject'; ru = 'Ошибка в обработчике события ПередУдалениемОбъекта';vi = 'Lỗi trong hàm sự kiện BeforeDeleteObject';ro = 'A apărut o eroare în handler-ul evenimentului BeforeDeleteObject'"));
	
	ErrorMessages.Insert(31, NStr("en = 'An error occurred in event handler BeforeProcessExportRule'; ru = 'Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';vi = 'Lỗi trong hàm sự kiện BeforeProcessExportRule';ro = 'A apărut o eroare handler-ul evenimentului BeforeProcessExportRule'"));
	ErrorMessages.Insert(32, NStr("en = 'An error occurred in event handler AfterDumpRuleProcessing'; ru = 'Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';vi = 'Lỗi trong hàm sự kiện AfterDumpRuleProcessing';ro = 'A apărut o eroare în handler-ul evenimentului AfterDumpRuleProcessing'"));
	ErrorMessages.Insert(33, NStr("en = 'An error occurred in event handler BeforeObjectExport'; ru = 'Ошибка в обработчике события ПередВыгрузкойОбъекта';vi = 'Lỗi trong hàm sự kiện BeforeObjectExport';ro = 'A apărut o eroare în handler-ul evenimentului BeforeObjectExport'"));
	ErrorMessages.Insert(34, NStr("en = 'An error occurred in event handler AfterObjectExport'; ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi = 'Lỗi trong hàm sự kiện AfterObjectExport';ro = 'A apărut o eroare în handler-ul evenimentului AfterObjectExport'"));
			
	ErrorMessages.Insert(41, NStr("en = 'An error occurred in event handler BeforeObjectExport'; ru = 'Ошибка в обработчике события ПередВыгрузкойОбъекта';vi = 'Lỗi trong hàm sự kiện BeforeObjectExport';ro = 'A apărut o eroare în handler-ul evenimentului BeforeObjectExport'"));
	ErrorMessages.Insert(42, NStr("en = 'An error occurred in event handler OnObjectExport'; ru = 'Ошибка в обработчике события ПриВыгрузкеОбъекта';vi = 'Lỗi trong hàm sự kiện OnObjectExport';ro = 'A apărut o eroare în handler-ul evenimentului OnObjectExport'"));
	ErrorMessages.Insert(43, NStr("en = 'An error occurred in event handler AfterObjectExport'; ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi = 'Lỗi trong hàm sự kiện AfterObjectExport';ro = 'A apărut o eroare în handler-ul evenimentului AfterObjectExport'"));
			
	ErrorMessages.Insert(45, NStr("en = 'Object conversion rule is not found'; ru = 'Не найдено правило конвертации объектов';vi = 'Không tìm thấy quy tắc chuyển đổi đối tượng';ro = 'Regula conversiei obiectului nu a fost găsită'"));
		
	ErrorMessages.Insert(48, NStr("en = 'An error occurred in event handler BeforeExportProcessor of the property group'; ru = 'Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';vi = 'Lỗi trong hàm sự kiện BeforeExportProcessor nhóm thuộc tính';ro = 'A apărut o eroare în handler-ul evenimentului BeforeExportProcessor din grupul de proprietăți'"));
	ErrorMessages.Insert(49, NStr("en = 'An error occurred in event handler AfterExportProcessor of the property group'; ru = 'Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';vi = 'Lỗi trong hàm sự kiện AfterExportProcessor nhóm thuộc tính';ro = 'A apărut o eroare în handler-ul evenimentului AfterExportProcessor din grupul de proprietate'"));
	ErrorMessages.Insert(50, NStr("en = 'Error in event handler BeforeExport (of collection object)'; ru = 'Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';vi = 'Lỗi trong hàm sự kiện BeforeExport (đối tượng tập hợp)';ro = 'Eroare la handler-ul de evenimente BeforeExport (a obiectului de colectare)'"));
	ErrorMessages.Insert(51, NStr("en = 'Error in event handler OnExport (of collection object)'; ru = 'Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';vi = 'Lỗi trong hàm sự kiện OnExport (đối tượng tập hợp)';ro = 'Eroare la handler-ul de evenimente OnExport (a obiectului de colectare)'"));
	ErrorMessages.Insert(52, NStr("en = 'Error in event handler AfterExport (of collection object)'; ru = 'Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';vi = 'Lỗi trong hàm sự kiện AfterExport (đối tượng tập hợp)';ro = 'Eroare la handler-ul de evenimente AfterExport (a obiectului de colectare)'"));
	ErrorMessages.Insert(53, NStr("en = 'An error occurred in global event handler BeforeObjectImporting (conversion)'; ru = 'Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';vi = 'Lỗi trong hàm sự kiện toàn cục BeforeObjectImporting (chuyển đổi)';ro = 'A apărut o eroare în handler-ul global a evenimentului BeforeObjectImporting (conversie)'"));
	ErrorMessages.Insert(54, NStr("en = 'An error occurred in global event handler AfterObjectImport (conversion)'; ru = 'Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';vi = 'Lỗi trong hàm sự kiện toàn cục AfterObjectImport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul global a evenimentului AfterObjectImport (conversie)'"));
	ErrorMessages.Insert(55, NStr("en = 'An error occurred in event handler BeforeExport (properties)'; ru = 'Ошибка в обработчике события ПередВыгрузкой (свойства)';vi = 'Lỗi trong hàm sự kiện BeforeExport (thuộc tính)';ro = 'A apărut o eroare în handler-ul evenimentului BeforeExport (proprietăți)'"));
	ErrorMessages.Insert(56, NStr("en = 'An error occurred in event handler OnExport (properties)'; ru = 'Ошибка в обработчике события ПриВыгрузке (свойства)';vi = 'Lỗi trong hàm sự kiện OnExport (thuộc tính)';ro = 'A apărut o eroare în handler-ul evenimentului OnExport (proprietăți)'"));
	ErrorMessages.Insert(57, NStr("en = 'An error occurred in event handler AfterExport (properties)'; ru = 'Ошибка в обработчике события ПослеВыгрузки (свойства)';vi = 'Lỗi trong hàm sự kiện AfterExport (thuộc tính)';ro = 'A apărut o eroare în handler-ul evenimentului AfterExport (proprietăți)'"));
	
	ErrorMessages.Insert(62, NStr("en = 'An error occurred in event handler BeforeDataExport (conversion)'; ru = 'Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';vi = 'Lỗi trong hàm sự kiện BeforeDataExport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului BeforeDataExport (conversie)'"));
	ErrorMessages.Insert(63, NStr("en = 'An error occurred in event handler AfterDataExport (conversion)'; ru = 'Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';vi = 'Lỗi trong hàm sự kiện AfterDataExport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului AfterDataExport (conversie)'"));
	ErrorMessages.Insert(64, NStr("en = 'An error occurred in global event handler BeforeObjectConversion (conversion)'; ru = 'Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';vi = 'Lỗi trong hàm sự kiện toàn cục BeforeObjectConversion (chuyển đổi)';ro = 'A apărut o eroare în handler-ul global a evenimentului BeforeObjectConversion (conversie)'"));
	ErrorMessages.Insert(65, NStr("en = 'An error occurred in global event handler BeforeObjectExport (conversion)'; ru = 'Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';vi = 'Lỗi trong hàm sự kiện toàn cục BeforeObjectExport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul global a evenimentului BeforeObjectExport (conversie)'"));
	ErrorMessages.Insert(66, NStr("en = 'An error occurred when receiving a subordinate object collection from the incoming data'; ru = 'Ошибка получения коллекции подчиненных объектов из входящих данных';vi = 'Lỗi nhận tập hợp đối tượng trực thuộc từ dữ liệu đến';ro = 'A apărut o eroare la primirea unei colecții de obiecte subordonate din datele primite'"));
	ErrorMessages.Insert(67, NStr("en = 'An error occurred when receiving the subordinate object properties from the incoming data'; ru = 'Ошибка получения свойства подчиненного объекта из входящих данных';vi = 'Lỗi nhận thuộc tính đối tượng trực thuộc từ dữ liệu đến';ro = 'A apărut o eroare la primirea proprietăților obiectului subordonat din datele primite'"));
	ErrorMessages.Insert(68, NStr("en = 'An error occurred when receiving the object properties from the incoming data'; ru = 'Ошибка получения свойства объекта из входящих данных';vi = 'Lỗi nhận thuộc tính đối tượng từ dữ liệu đến';ro = 'A apărut o eroare la primirea proprietăților obiectului din datele intrate'"));
	
	ErrorMessages.Insert(69, NStr("en = 'An error occurred in global event handler AfterObjectExport (conversion)'; ru = 'Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';vi = 'Lỗi trong hàm sự kiện toàn cục AfterObjectExport (chuyển đổi)';ro = 'A apărut o eroare în handler-ul global a evenimentului AfterObjectExport (conversie)'"));
	
	ErrorMessages.Insert(71, NStr("en = 'Match for the Source value is not found'; ru = 'Не найдено соответствие для значения Источника';vi = 'Không tìm thấy sự tương ứng đối với giá trị Nguồn';ro = 'Coincidența pentru valoarea sursă nu a fost găsită'"));
	
	ErrorMessages.Insert(72, NStr("en = 'An error occurred when exporting data for the exchange plan node'; ru = 'Ошибка при выгрузке данных для узла плана обмена';vi = 'Lỗi khi kết xuất dữ liệu đối với nút của sơ đồ trao đổi';ro = 'A apărut o eroare la exportul datelor pentru nodul planului de schimb'"));
	
	ErrorMessages.Insert(73, NStr("en = 'An error occurred in event handler SearchFieldsSequence'; ru = 'Ошибка в обработчике события ПоследовательностьПолейПоиска';vi = 'Lỗi trong hàm sự kiện SearchFieldsSequence';ro = 'A apărut o eroare în handler-ul evenimentului SearchFieldsSequence'"));
	ErrorMessages.Insert(74, NStr("en = 'Import exchange rules for data export again.'; ru = 'Необходимо перезагрузить правила обмена для выгрузки данных.';vi = 'Cần kết nhập lại quy tắc trao đổi để kết xuất dữ liệu.';ro = 'Importați din nou regulile de schimb pentru exportul de date.'"));
	
	ErrorMessages.Insert(75, NStr("en = 'An error occurred in event handler AfterImportOfExchangeRules (conversion)'; ru = 'Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';vi = 'Lỗi trong hàm sự kiện AfterImportOfExchangeRules (chuyển đổi)';ro = 'A apărut o eroare la handler-ul evenimentului AfterImportOfExchangeRules (conversie)'"));
	ErrorMessages.Insert(76, NStr("en = 'An error occurred in event handler BeforeSendingUninstallInformation (conversion)'; ru = 'Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)';vi = 'Lỗi trong hàm sự kiện BeforeSendingUninstallInformation (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului BeforeSendingUninstallInformation (conversie)'"));
	ErrorMessages.Insert(77, NStr("en = 'An error occurred in event handler OnObtainingInformationAboutDeletion (conversion)'; ru = 'Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';vi = 'Lỗi trong hàm sự kiện OnObtainingInformationAboutDeletion (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului OnObtainingInformationAboutDeletion (conversie)'"));
	
	ErrorMessages.Insert(78, NStr("en = 'An error occurred when executing the algorithm after import of the parameter values'; ru = 'Ошибка при выполнении алгоритма после загрузки значений параметров';vi = 'Lỗi khi thực hiện thuật toán sau khi kết nhập giá trị tham số';ro = 'A apărut o eroare la executarea algoritmului după importul valorilor parametrilor'"));
	
	ErrorMessages.Insert(79, NStr("en = 'An error occurred in event handler AfterObjectExportToFile'; ru = 'Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';vi = 'Lỗi trong hàm sự kiện AfterObjectExportToFile';ro = 'A apărut o eroare în handler-ul evenimentului AfterObjectExportToFile'"));
	...
	ErrorMessages.Insert(80, NStr("en = 'Error of the predefined item property setting.
		...
		|vi = 'Lỗi thiết lập thuộc tính của phần tử định trước.
		|Không nên đánh dấu xóa phần tử định trước. Việc đánh dấu xóa cho đối tượng không được thiết lập.';
		|ro = 'Eroare la setarea elementului proprietății predefinite.
		|Nu puteți marca elementul predefinit pentru ștergere. Marcarea pentru ștergere obiecte nu este instalată.'"));
	//
	ErrorMessages.Insert(83, NStr("en = 'An error occurred while accessing the object tabular section. The object tabular section cannot be changed.'; ru = 'Ошибка обращения к табличной части объекта. Табличная часть объекта не может быть изменена.';vi = 'Lỗi truy cập đến phần bảng đối tượng. Không thể thay đổi phần bảng đối tượng.';ro = 'A apărut o eroare la accesarea secțiunii tabulare a obiectului. Secțiunea tabulară a obiectului nu poate fi modificată.'"));
	ErrorMessages.Insert(84, NStr("en = 'Collision of change closing dates.'; ru = 'Коллизия дат запрета изменения.';vi = 'Mâu thuẫn ngày cấm thay đổi.';ro = 'Coliziunea datei de închidere a modificării.'"));
	
	ErrorMessages.Insert(173, NStr("en = 'An error of the exchange node lock occurred. Maybe, data synchronization is already in progress'; ru = 'Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';vi = 'Lỗi phong tỏa nút trao đổi. Có thể, đã thực hiện đồng bộ hóa dữ liệu';ro = 'A apărut o eroare a blocării nodului de schimb. Poate că sincronizarea datelor este deja în curs'"));
	ErrorMessages.Insert(174, NStr("en = 'Exchange message was previously received'; ru = 'Сообщение обмена было принято ранее';vi = 'Đã tiếp nhận thông điệp trao đổi trước đây';ro = 'Mesajul de schimb a fost primit anterior'"));
	ErrorMessages.Insert(175, NStr("en = 'An error occurred in event handler BeforeModifiedObjectsReceiving (conversion)'; ru = 'Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';vi = 'Lỗi trong hàm sự kiện BeforeModifiedObjectsReceiving (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului BeforeModifiedObjectsReceiving (conversie)'"));
	ErrorMessages.Insert(176, NStr("en = 'An error occurred in event handler AfterGettingInformationAboutExchangeNodes (conversion)'; ru = 'Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';vi = 'Lỗi trong hàm sự kiện AfterGettingInformationAboutExchangeNodes (chuyển đổi)';ro = 'A apărut o eroare în handler-ul evenimentului AfterGettingInformationAboutExchangeNodes (conversie)'"));
		
	ErrorMessages.Insert(1000, NStr("en = 'An error occurred when creating a temporary file of data export'; ru = 'Ошибка при создании временного файла выгрузки данных';vi = 'Lỗi khi tạo tệp tạm thời kết xuất dữ liệu';ro = 'A apărut o eroare la crearea unui fișier temporar de export de date'"));
